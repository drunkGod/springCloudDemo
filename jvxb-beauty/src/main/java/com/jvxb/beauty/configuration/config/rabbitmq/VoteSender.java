...
public class VoteSender implements RabbitTemplate.ConfirmCallback {
    ...
    public void confirm(CorrelationData correlationData, boolean isSendSuccess, String error) {
        ...
        if (isSendSuccess) {
            System.out.println(String.format("确认【发送】到MQ：【%s】，发送内容: 【%s】", MqConstant.VOTE_DIRECT_EXCHANGE, correlationData.getId()));
        }

        if (!isSendSuccess) {
            System.out.println(String.format("确认【未发送】到MQ：【%s】，发送内容: 【%s】", MqConstant.VOTE_DIRECT_EXCHANGE, correlationData.getId()));
        }
