 ...
 * @since 2020-06-13
 */
public class EsConditionResolve {

    private static final Logger logger = LoggerFactory.getLogger(EsConditionResolve.class);
    ...
    public static void resolveQuery(SearchRequestBuilder searchRequestBuilder,
        ...
        for (Map.Entry<String, Object> entry : conditionMap.entrySet()) {
            ...
            if (type.equalsIgnoreCase(EsKeyword.FIELD.getKeyword())) {
                ...
                resolveQueryLimit(searchRequestBuilder, value);
            ...
            } else if (type.equalsIgnoreCase(EsKeyword.SELECT.getKeyword())) {
    ...
    private static void resolveQueryFetchSource(SearchRequestBuilder searchRequestBuilder, Object value) {
        ...
        if (value instanceof CharSequence) {
            String[] includes = value.toString().split(",");
            searchRequestBuilder.setFetchSource(includes, null);
        } else if (value instanceof Map) {
            Object includeValue = ((Map) value).get(EsKeyword.SELECT_INCLUDES.getKeyword());
            Object excludeValue = ((Map) value).get(EsKeyword.SELECT_EXCLUDES.getKeyword());
            String[] includes = includeValue.toString().split(",");
            String[] excludes = excludeValue.toString().split(",");
            searchRequestBuilder.setFetchSource(includes, excludes);
        }
    }

    private static void resolveQueryLimit(SearchRequestBuilder searchRequestBuilder, Object value) {
        //处理分页 "page": "1,10"
    ...
    private static void resolveQueryField(BoolQueryBuilder boolQueryBuilder, Object value) {
        ...
        for (Map.Entry<String, Object> entry : ((Map<String, Object>) value).entrySet()) {
            try {
                ...
                String keyword = handleStr.split("\\s+", 2)[0];
                String fieldValue = handleStr.split("\\s+", 2)[1];
                if (keyword.equalsIgnoreCase(EsKeyword.FIELD_EQ.getKeyword())) {
                    boolQueryBuilder.filter(!field.contains(COMMA) ? QueryBuilders.termQuery(field, fieldValue) : QueryBuilders.multiMatchQuery(fieldValue, field.split(COMMA)));
                } else if (keyword.equalsIgnoreCase(EsKeyword.FIELD_NE.getKeyword())) {
                    boolQueryBuilder.mustNot(QueryBuilders.termQuery(field, fieldValue));
                } else if (keyword.equalsIgnoreCase(EsKeyword.FIELD_LIKE.getKeyword())) {
                    Arrays.stream(field.split(COMMA)).forEach(fd -> boolQueryBuilder.filter(QueryBuilders.wildcardQuery(fd, String.format("*%s*", fieldValue))));
                }
            } catch (Exception e) {
    ...
    private static void resolveHighLevelQueryFetchSource(SearchSourceBuilder sourceBuilder, Object value) {
        Object includeValue = ((Map) value).get(EsKeyword.SELECT_INCLUDES.getKeyword());
        Object excludeValue = ((Map) value).get(EsKeyword.SELECT_EXCLUDES.getKeyword());
        String[] includes = Optional.ofNullable(includeValue).map(e -> e.toString().split(",")).orElse(null);
        String[] excludes = Optional.ofNullable(excludeValue).map(e -> e.toString().split(",")).orElse(null);
